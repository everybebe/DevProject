...
public class ValidationController {
	 ...
	 * 4. 에러를 받을 BindingResult를 설정한다. (컨트롤러 메소드 내에 설정한다)
	 *  
	 * 	
	 */
	...
	public String registerValidationForm02Result(@Validated Member member, BindingResult result) {
		log.info("registerValidationForm02Result");
		log.info("result.hasError() : " + result.hasErrors());
		
		if(result.hasErrors()) { //에러 정보가 존재
			List<ObjectError> allErrors = result.getAllErrors();
			List<ObjectError> globalErrors = result.getAllErrors();
	 ...
	 *      	>@Size(max=, min=)
	 *      @Max(value=)	   : value 보다 작거나 같은걸 검사한다.
	 *      @Min(value=)       : value 보다 크거나 같은걸 검사한다.
	 *      @Email		       : email 주소 형식인지 검사한다.
	 *      @Past	   		   : 과거 날짜인지를 검사한다.
	...
	public String registerValidationForm03(@Validated Member member, BindingResult result) {
