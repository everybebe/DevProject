...
public class DatabaseConnectcontroller {
	 ...
	 *   	3. 게시판 구현
	 *   
	 *   		기본적인 JDBC를 이용한 디비 설계는 DAO란에서 sql 쿼리가 노출되어 개발하는 방식이다보니 우리가 중급프로젝트때 또는 JSP에서
	 *   		preparestatement 방식의 데이터베이스 연동과 다를바 없는 형태로, 게시판 구현을 진행하지 않고 내용만 간략히 보고 넘어가도록 한다.
	 *   
	 ...
	 *   	발생하지만 JPA는 필드만 추가 후 테이블 정보과 매핑만 시켜주면 JPA가 자동으로 분석을 진행한다.
	 *   	그리고, 사용하는 데이터베이스 종류가 변경된다 하더라도 문제가 없다. (JPA는 SQL을 직접 건들이지 않고 함수와 같은 기능들로만 쿼리를 요청하고
	 *   	데이터베이스와 통신하기 때문입니다.)
	 *   
	 *   	JPA는 JPA구현체인 hibernate를 이용하여 사용합니다. 그러기 위해선 관련 의존관계를 등록 후 사용해야 합니다.
	 *   	SQL문을 이용하여 데이터베이스를 연동하여 데이터를 가공하는게 아닌, Method를 통해 데이터베이스를 조작할 수 있다는 장점이 있어 객체 모델을
	 ...
	 *   	- 저는 ibatis를 사용했습니다, 저는 mybatis를 사용했습니다 와 같은 대답을 하시면 됩니다.
	 *   
	 *   ----------------------------------------------------
	 *   
	 *   7. 마이바티스
