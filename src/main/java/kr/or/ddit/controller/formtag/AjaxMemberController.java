package kr.or.ddit.controller.formtag;
...
public class AjaxMemberController {
	...
	public ResponseEntity<String> ajaxRegister03(
			@RequestBody Member member){ //@RequestBody 클라이언트는 요청의 본문에 데이터를 담아 전송
	...
	public ResponseEntity<String> register05(String userId, String password) {
		...
		log.info("userId : " + userId);
		log.info("password : " + password);  //요청본문은 가져오지만 requestBody 가 없어서 요청데이터를 못가져옴
		return new ResponseEntity<String>("SUCCESS", HttpStatus.OK);
	}
	...
	public ResponseEntity<String> register06(
		...
		log.info("userId : " + userId);
		log.info("member.userId : " + member.getUserId());  //요청본문은 가져오지만 requestBody 가 없어서 요청데이터를 못가져옴
		log.info("member.password : " + member.getPassword());  //요청본문은 가져오지만 requestBody 가 없어서 요청데이터를 못가져옴
		return new ResponseEntity<String>("SUCCESS", HttpStatus.OK);
	...
	public ResponseEntity<String> register07(
		...
		//동기 처리시에는 컨트롤러 메소드 내에서 List타입으로 값을 바인딩할 수 없지만, 객체 내 존재하는 List타입으로는 데이터를 바인딩할 수 있다.
