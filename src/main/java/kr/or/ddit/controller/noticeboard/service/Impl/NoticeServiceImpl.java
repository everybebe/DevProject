...
public class NoticeServiceImpl implements INoticeService{
	...
	public ServiceResult insertNotice(HttpServletRequest req, NoticeVO noticeVO) {
		...
		if(status > 0) {
			...
			try {
				tst.sendGet("406호 학생", noticeVO.getBoTitle());
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if(status > 0) {
			...
			try {
				...
				if(delNoticeNo != null) {
					...
					// 	  데이터베이스에서 데이터를 삭제하고 서버 업로드 경로에 업로드 되어 있는 파일 데이터를 삭제한다.
	...
	public static void deleteFolder(HttpServletRequest req, String path) {
		...
		if(folder.exists()) {
			...
			for (int i = 0; i < fileList.length; i++) {
				if (fileList[i].isFile()) { //폴더안의 있는 파일이 파일일 때
					fileList[i].delete();
					}else {					//폴더 안에 있는 파일이 폴더일 때 재귀함수 호출(폴더안으로 들어갈라고)
