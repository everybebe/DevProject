...
public class NoticeProfileController {
	...
	public String noticeProfile(
		...
		String goPage = "";
		/*
		 * HttpSession session = req.getSession(); DDITMemberVO sessionMember =
		 * (DDITMemberVO) session.getAttribute("SessionInfo");
		 * 
		 * if(sessionMember == null) { ra.addFlashAttribute("message",
		 * "로그인 후 이용가능합니다!"); return "redirect:/notice/login.do"; }
		 */
