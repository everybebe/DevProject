...
public class TransactionController {
	 ...
	 * 
	 * 		서브시 구현 객체에 트랜잭션 적용
	 * 		- 회원 정보를 저장하다가 실패하거나 회원 권한 정보를 저장하다가 실패하면 회원 테이블과 회원 권한 테이블 모두 저장되지 않는다.
	 ...
	 * 		실제로 '트랜잭션 롤백 처리', 'checked Exception vs unchecked Exception' 등 검색 해보면 잘못 된 정보들로 복사/붙여놓기 되어 있는 내용이
	 * 		수두룩 하여 많은 에러 정보를 공유하고 있다. (모든 블로그가 그렇진 않지만, 대표적인 표가 무수히 많이 공유되고 있다)
	 * 
	 * 		***
	 * 		* 예외처리 시 트랜잭션 처리에 대한 정보가 잘못되어 표기된 경우가 많다.
	 * 		이럴때 되고 저럴때 되고가 아니라, 기본적은 스프링 트랜잭션의 정책이 있고 그 안에서 정책 이 외에 옵션으로 특정 에러가 발생했을 때
	 * 		롤백을 진행할 수 있도록 제공한다. (많은 블로그들을 100% 믿지 말길 바랍니다.. 혹여나 블로그를 작성한다면 검증된 내용으로만 적어주길 바랄게요,,)
	 * 
	 * 		Exception(예외)와 Error(에러)
	 * 		- Exception : 개발 알고리즘에서 발생하는 오류로 개발자가 작성한 ㄴ코드에서 발생하므로 예외를 상황에 맞춰 처리할 수 있다.
	 * 		- Error : 시스템에서 발생하는 심각한 수준의 에러로 개발자가 미리 예측하여 대응할 수 없기 때문에 예외 처리에 대한 부분을 신경쓰지 않아도 된다.
	 * 
	 * 		CheckedExcepiton과 UnCheckedException
	 ...
	 * 		 클래스 		  | IOExceptin, SQLException...  |    NullPointException, IndexOutOfBoundException...
	 * 	─────────────────────────────────────────────────────────────────────────────────────────────────────────  
	 * 
	 * 	***** 트랜잭션도 AOP의 개념이 반영된 관점 지향 프로그래밍이라 할 수 있습니다.
	 * 	
	 * 	3-1) 선언적 트랜잭션(@Transactional)
	 * 		- 컨드롤러 메소드 각 단위로 세밀한 트랜잭션 속성 제어가 가능
	 ...
	 * 			- REPEATBLE_READ : Dirty read, Non-repeatable Read가 방지됨을 나타내는 상수입니다.
	 ...
	 * 
	 * 			2) propagation (전파옵션)
	 * 				- 기존 진행중인 트랜잭션 외에 추가적으로 진행중인 트랜잭션이 존재할 때 추가적인 트랜잭션에 대해서 어떠헥 처리할지에 대한 설정
	 * 				- 추가적인 트랜잭션을 기존 트랜잭션에 포함 시켜 함께 처리할 수도 있고, 추가적인 트랜잭션처럼 별도의 트랜잭션으로 추가할 수도 있고
	 * 					다른 트랜잭션처럼 진행되다 에러를 발생 시킬 수도 있다.
	 * 	
	 * 				2-1) 옵션
	 * 				
	 ...
	 * 						> 부모에서 예외가 발생했을 떄, 자식까지 영향이 가서 Commit되지 않는다.
