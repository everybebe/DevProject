...
public class AOPController {
	 ...
	 * 		성희 팀장님은 아주 긍정적인 검토안을 정수학생한테 전달하면서 우리 서비스 전체에도 각 처리속도를 찍어주세요 라고 다시 부탁합니다.
	 * 		정수학생은 본인이 만들어 낸 메소드를 각 기능별 서비스 로직에 하나하나씩 약 2만개쯤 넣을 쯤에 의문을 갖습니다..
	 * 
	 ...
	 * 
	 * 		이러한 인프라 로직은 아래처럼 하나의 관심사를 가질 수 있는데 이런 관심사들의 중복이 횡단으로 나타나는데, 이것을 가르켜 '횡단 관심사(Corss-Cutting Concern)'라고 함.
	 * 
	 * 			┌──────────────────────────────────────────────────────────────────────────┐
	 ...
	 * 		로그를 출력하는 동의 이벤트를 실행한다.
	 * 
	 * 		[AOP 프록시]
	 * 		클라이언트가 요청한 해당 요청을 서비스 로직으로 전달되기 전 해당 비즈니스 로직 부를 실제 대상인것처럼 위장해서 클라이언트의 요청을 받는다.
	 * 		AOP에서 프록시 개념은 대리자라고 생각하면 됩니다.
	 * 		우리는 서비스에서 이뤄지는 전역의 비즈니스 로직들에서관심사별로 위빙을 지정해 해당 서비스의 비즈니스 로직이 실행될 때,
	 * 		인터페이스 기반의 포륵시를 이용하여 설정된 횡단 관심사들을 실행합니다.
	 * 
