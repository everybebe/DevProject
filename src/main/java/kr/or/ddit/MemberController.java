...
public class MemberController {
	...
	@RequestMapping(value="/register12", method = RequestMethod.GET)
	...
	//클라이언트에서 선언된 필드명은 userId인데 서버 컨트롤러 메소드에서 받는 필드명이 ussername이면 파라미터를 받을 수 없다.
	...
	//3) 여러 개의 폼 텍스트 필드 요서값을 매개변수 순서와 상관없이 매개변수명을 기준으로 처리한다.
	...
	//7) 복수 선택이 가능한 폼 셀렉트 박스 요소값을 기본 문자열 배열 타입 매개변수로 처리한다.
	...
	public String registerMultiSelect02(String[] carArray) {
		...
		if(carArray != null) {
			log.info("carArray.length : " + carArray.length); //선택된 값들 배열 길이,  1개 선택 시 carArray.length : 1, 3개 선택 시 carArray.length : 3
	...
	public String registerUserAddress(Member member) {
		...
		if(address != null) {
		log.info("member.getAddress().getProstCode() : " + address.getPostCode());
	...
	public String registerDate01(Date dateOfBirth) {
		// 결과로 Date 타입의 값을 받기 위해서는 default인 2023/05/24의 형태로 값을 설정해서 보내야 한다.
		// 내가 원하는 Date타입의 형식이 존재한다면 @DateTimeFormat pattern 속성을 이용하여
		// 원하는 패턴을 설정하여 날짜 형식의 값을 넘겨받는다 (pattern = "yyyMMdd", pattern ="yyyy-MM-dd")
	...
	public String registerUser(MemberAllForm member, Model model) {
		...
		if(cardList != null) {
			...
			for (int i = 0; i < cardList.size(); i++) {
				...
				log.info("카드-유효년월 : " + card.getValidMonth());
			}
		} else {
			log.info("cardList is null");
		}
	...
	public String registerFile06(List<MultipartFile> pictureList) {
		...
		log.info("registerList.size() : " + pictureList.size());
   ...
   public String registerFile07(MultiFileMember MultipartFile) {
      log.info("registerFile07실행...!");
      List<MultipartFile> pictureList = MultipartFile.getPictureList();
      log.info("prictureList.size() : " + pictureList.size());
